...
Browser tests use anonymous demo identities and events. Published screenshots in `docs/assets/responsive-calendar/` are generated from these demo fixtures in fresh browser contexts with API responses mocked. They use generic demo names, events and locations, initials instead of profile photos, and are visually reviewed before publication. New copy is provided in German and English, with the existing locale fallback for other languages. These checks do not claim physical iPhone or Safari validation.
