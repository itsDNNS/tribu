...
export default function EventEditor({
  ...
  const [advanced, setAdvanced] = useState(
    Boolean(recurrence || icon || allDay || (date && endDate !== date)),
  );
  const changeDate = (value) => {
    const shifted = retargetCreateDraft(
  ...
  const toggleMember = (id) =>
    setAssigned((previous) => {
      const current = previous.includes('all')
        ? members.map((m) => Number(m.user_id))
        : previous.map(Number);
  ...
  return (
    <CalendarDialog
      title={
        editing
          ? t(messages, 'module.calendar.edit_event')
          : cal.duplicateSource
    ...
    >
      <form id="calendar-event-form" onSubmit={submit}>
        <fieldset className="tc-form-grid" disabled={busy}>
          {editing && cal.editingEvent?.is_recurring && (
            <p className="tc-form-hint full">
              {t(messages, 'module.calendar.edit_recurring_warning')}
            </p>
          )}
          {cal.duplicateSource && !editing && (
            <p className="tc-form-hint full">
              {t(messages, 'module.calendar.duplicating_hint').replace(
                '{title}',
                cal.duplicateSource.title,
              )}
            </p>
          )}
          ...
          <label className="tc-field">
            ...
            <input
              ...
              onChange={(e) => setStartsAt(`${date}T${e.target.value}`)}
            />
          </label>
          <label className="tc-field">
            {copy('until')}
            <input
              ...
              onChange={(e) =>
                setEndsAt(e.target.value ? `${endDate}T${e.target.value}` : '')
              }
            />
          </label>
                ...
                <label key={member.user_id}>
                  <input
                    ...
                    checked={
                      ...
                      assigned.map(Number).includes(Number(member.user_id))
                    }
          ...
          <div className="tc-field full">
            ...
            <div className="tc-colors">
              <button
                type="button"
                className="tc-color-auto"
                aria-label={copy('automatic_color')}
                aria-pressed={!color}
                onClick={() => setColor('')}
              >
                <span
                  style={calendarEventStyle(
                    {
                      assigned_to: assigned.includes('all') ? 'all' : assigned,
                    },
                    members,
                  )}
                />
                {copy('automatic_color')}
              </button>
              {[
                ...new Set([
                  ...CALENDAR_COLORS,
                  ...(color && !CALENDAR_COLORS.includes(color) ? [color] : []),
                ]),
              ].map((c) => (
                <button
                  type="button"
          ...
          >
            ...
            <div className="tc-form-grid">
              ...
              <label className="tc-check full">
                <input
                  type="checkbox"
                  checked={assigned.includes('all')}
                  onChange={(e) => setAssigned(e.target.checked ? ['all'] : [])}
                />
                {copy('all_members')}
              </label>
              <label className="tc-field full">
                {t(messages, 'module.calendar.recurring')}
                <select
                  aria-label={t(messages, 'module.calendar.recurring')}
                  value={recurrence}
              ...
              <label className="tc-field full">
                {t(messages, 'module.calendar.icon')}
                <select
                  aria-label={t(messages, 'module.calendar.icon')}
                  value={icon}
                  onChange={(e) => setIcon(e.target.value)}
                >
                  <option value="">
                    {t(messages, 'module.calendar.icon_none')}
                  </option>
                  {CALENDAR_EVENT_ICON_OPTIONS.map((o) => (
          ...
          {error && (
            <p role="alert" className="tc-error full">
              {error}
            </p>
          )}
        </fieldset>
