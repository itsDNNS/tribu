...
export default function CalendarView(props) {
  ...
  const eventButton = event => {
    ...
    return <button type="button" key={`${event.id}:${event.starts_at}`} className="tc-calendar-event" style={calendarEventStyle(event, members)} onClick={()=>setModal({kind:'event',event})} title={`${event.title} · ${time(event.starts_at)}`}>
