...
export default function FamilyTopbar({
  ...
  const greeting = t(
    messages,
    `module.dashboard.greeting_${hour < 12 ? 'morning' : hour < 18 ? 'afternoon' : 'evening'}`,
  );
  return (
    <div className="today-command-header tc-topbar">
      <div>
        <div className="today-command-title">
          {greeting}, <span>{me?.display_name}</span>
        </div>
        <p className="today-command-family">
          ...
          <time className="today-command-clock" dateTime={now.toISOString()}>
      ...
      <div className="dashboard-header-actions">
        {onOpenSearch && (
          <button className="dashboard-search-btn" onClick={onOpenSearch}>
            <Search size={16} />
        ...
        {onOpenNotifications && (
          ...
          >
            <Bell size={17} />
            {unreadCount > 0 && (
              <span className="dashboard-action-badge">{unreadCount}</span>
            )}
          </button>
        )}
        {setTheme && (
          <button
            className="dashboard-icon-action"
            aria-label={t(messages, 'module.dashboard.toggle_theme')}
            onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}
          >
            {theme === 'light' ? <Sun size={20} /> : <Moon size={20} />}
          </button>
        )}
        <span className="tc-topbar-note">
          {t(messages, 'module.dashboard.family_note')} <span>♥</span>
        </span>
      </div>
    </div>
  );
